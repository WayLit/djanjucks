...
class DjanjucksCompiler extends Compiler {
  ...
  compileCycle(node, frame) {
    ...
    if (args.length === 1 && args[0] instanceof nodes.Symbol) {
      ...
      this._emitLine(
        `throw new Error('cycle: No named cycles in template. "${args[0].value}" is not defined.');`
      )
      this._emitLine(`}`)
    ...
    this._emitLine(
      `${this.buffer} += runtime.suppressValue(${id}.current, env.opts.autoescape)`
    )
    this._emitLine(`}`)
  ...
  compileResetCycle(node, frame) {
    ...
    if (args.length < 1) {
      ...
      this._emitLine(`if (!${id}) {`)
      this._emitLine(
        `throw new Error('resetcycle: No named cycle in template called "${args[0].value}".');`
      )
      this._emitLine(`}`)
    }
